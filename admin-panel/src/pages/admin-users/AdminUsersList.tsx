...
const ADMIN_USER_CREATE_ROLE_FALLBACKS = ["YARD_SUPERVISOR"];
...
const sortRoleOptions = (roles: string[]) => {
  return [...roles].sort((a, b) => {
    ...
    return formatRoleLabel(a).localeCompare(formatRoleLabel(b));
  });
};
...
const loadRoles = useCallback(async () => {
  ...
  try {
    ...
    // Add active role-master-backed operational roles that may not yet be returned
    // by older backend/admin-role APIs. This fixes missing Yard Supervisor in Create Admin User.
    roles = Array.from(new Set([...roles, ...ADMIN_USER_CREATE_ROLE_FALLBACKS]));

    // Org admin can create/manage operational users, but should not create another super/admin role.
    if (!isSuper) {
      roles = roles.filter((role) => role !== "SUPER_ADMIN" && ORG_ADMIN_ALLOWED_ROLES.has(role));
    }

    setRoleOptions(sortRoleOptions(roles));
  } catch (e) {
    console.error("[admin_users] loadRoles", e);
  }
}, [language, isSuper]);
