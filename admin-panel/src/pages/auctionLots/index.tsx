...
const LANE_KEY_ALIASES: Record<string, string> = {
...
function normalizeLaneKey(value: any): string {
...
const normalizeLaneSessionId = (lane: any): string => String(
  lane?._id
  || lane?.id
  || lane?.session_id
  || lane?.auction_session_id
...
export const AuctionLots: React.FC = () => {
  ...
  const [lanePreviewLoading, setLanePreviewLoading] = useState(false);
  const [lanePreviewError, setLanePreviewError] = useState<string | null>(null);
  const [lotOptions, setLotOptions] = useState<LotOption[]>([]);
  const [selectedLot, setSelectedLot] = useState<any | null>(null);
  const lanePreviewRequestIdRef = useRef(0);
  ...
  useEffect(() => {
    console.info("[CreateAuctionLot][submitDisabledReason]", {
      ...
      selected_lane_id: createForm.session_id || null,
      auto_assign_lane: Boolean(createForm.auto_assign_lane),
    });
  }, [submitDisabledReason, createSubmitDisabled, createForm.session_id, createForm.auto_assign_lane]);
  ...
  const buildSessionOptionsForLot = (lot: any, sessions: any[]) => {
    ...
    const allowedStatuses = new Set(["PLANNED", "LIVE", "READY_TO_CLOSE", "PAUSED", "OPEN"]);
    ...
    const wantedCommodityKey = resolveCommodityMatchValue(lot);
    ...
    const ranked = sessions
      .map((s) => {
        ...
        const isCommodityLane = laneType === "COMMODITY_LANE";
        const isGeneralLane = laneType === "GENERAL_LANE";
        const isOverflowLane = Boolean(s?.is_overflow_lane);
        const laneCommodityKey = resolveCommodityMatchValue(s);
        const commodityGroupMatch = Boolean(wantedCommodityKey && laneCommodityKey && wantedCommodityKey === laneCommodityKey);
    ...
    if (import.meta.env.DEV) {
      console.debug("[AUCTION_LOTS][laneRankingUI]", {
        ...
        candidates: ranked.map((entry) => ({
          ...
          commodity_group_match: Boolean(entry?.compatible && entry?.rank === 0),
          reason: entry.why,
        })),
      });
    }
  ...
  const fetchLanePreviewForSelectedLot = async (lot: any, opts?: { includeLotId?: boolean }) => {
    const username = currentUsername();
    if (!username || !lot || !canSessionsList) {
      setLanePreviewLoading(false);
      setLanePreviewError(!canSessionsList ? "Missing permission: auction_sessions.list (VIEW)." : null);
      return { sessions: [] as any[], options: [] as SessionOption[] };
    }
    const requestId = ++lanePreviewRequestIdRef.current;
    setLanePreviewLoading(true);
    setLanePreviewError(null);
    setSessionOptions([]);
    const includeLotId = opts?.includeLotId !== false;
    try {
      const resp = await getAuctionSessions({
        username,
        language,
        filters: {
          org_id: lot?.org_id || undefined,
          mandi_id: lot?.mandi_id ?? lot?.mandi_id_value ?? (filters.mandi_code ? Number(filters.mandi_code) : undefined),
          page_size: 100,
          usable_for_mapping: true,
      ...
      setSessionItems([]);
      setSessionOptions([]);
      setLanePreviewError("Unable to check lane assignment. Please refresh or create lane manually.");
      return { sessions: [] as any[], options: [] as SessionOption[] };
    } finally {
      if (requestId === lanePreviewRequestIdRef.current) {
        setLanePreviewLoading(false);
      }
    }
  };
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        auto_assign_lane: Boolean(createForm.auto_assign_lane),
        session_id: selectedLaneIdForSubmit || undefined,
      ...
      console.info("[CreateAuctionLot][productTiming]", {
        ...
        lane_session_id: selectedLaneIdForSubmit || null,
      ...
      if (String(createdItem?.queue_reason || "").toUpperCase() === "CAPACITY_CAP") {
        ...
        const autoAssignPrefix = createForm.auto_assign_lane
          ? `Auto-assigned to ${assignedLane}. Reason: Commodity group match. `
          : "";
        setCreateSuccess(
          `${autoAssignPrefix}Assigned Lane: ${assignedLane} | Lane Type: ${assignedLaneType} | Queue Position: ${queuePos} | Queue Reason: ${queueReasonMsg} | Auto-start: ${willAutoStart}`,
        );
      }
      setOpenCreate(false);
  ...
  const handleCreateAndAssignLane = async () => {
    ...
    try {
      ...
      let refreshed = await fetchLanePreviewForSelectedLot(selectedLot, { includeLotId: true });
      let refreshedOptions = refreshed.options;
      console.info("[CreateAuctionLot][compatibleLaneApiResponse]", {
        pass: "with_lot_id",
        count: refreshedOptions.length,
      });
      let hasCreatedLaneInOptions = Boolean(
        createdLaneId
        && refreshedOptions.some((opt) => String(opt?.value || "") === createdLaneId),
      );
      // Fallback re-fetch without lot_id filter in case usable_for_mapping backend filter is strict.
      if (createdLaneId && !hasCreatedLaneInOptions) {
        refreshed = await fetchLanePreviewForSelectedLot(selectedLot, { includeLotId: false });
        refreshedOptions = refreshed.options;
        console.info("[CreateAuctionLot][compatibleLaneApiResponse]", {
          pass: "without_lot_id",
          count: refreshedOptions.length,
        });
        hasCreatedLaneInOptions = refreshedOptions.some((opt) => String(opt?.value || "") === createdLaneId);
      }
      console.info("[CreateAuctionLot][laneRefreshAfterCreate]", {
        created_lane_id: createdLaneId || null,
        created_lane_name: createdLaneName,
        selected_lot_commodity_group: selectedLot?.commodity_group || selectedLot?.commodity_name || selectedLot?.commodity_name_en || null,
        selected_lot_commodity_group_code: selectedLot?.commodity_group_code || selectedLot?.commodity_code || null,
  ...
  useEffect(() => {
    if (!openCreate) return;
    if (!createForm.auto_assign_lane || !selectedLot) {
      setLanePreviewLoading(false);
      setLanePreviewError(null);
      return;
    }
    console.log("[autoAssignLane][sourceLotChanged]", {
      selected_lot_code: selectedLot?.lot_code || selectedLot?.token_code || selectedLot?._id || null,
      selected_commodity_group: selectedLot?.commodity_group || selectedLot?.commodity_name || selectedLot?.commodity_name_en || null,
      selected_product_name: selectedLot?.product_name_en || selectedLot?.commodity_product_name_en || selectedLot?.product || null,
      auto_assign_enabled: Boolean(createForm.auto_assign_lane),
    });
    void fetchLanePreviewForSelectedLot(selectedLot);
  }, [
    openCreate,
    createForm.auto_assign_lane,
    selectedLot?._id,
    ...
    selectedLot?.mandi_id,
    selectedLot?.mandi_id_value,
    filters.mandi_code,
  ]);
          ...
          <DialogContent>
            ...
            <Stack spacing={2} mt={1}>
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                ...
                <Autocomplete
                  ...
                  onChange={(_e, option) => {
                    ...
                    setLanePreviewError(null);
                    console.log("[autoAssignLane][sourceLotChanged]", {
                      selected_lot_code: option?.lot?.lot_code || option?.lot?.token_code || option?.lot?._id || null,
                      selected_commodity_group: option?.lot?.commodity_group || option?.lot?.commodity_name || option?.lot?.commodity_name_en || null,
                      selected_product_name: option?.lot?.product_name_en || option?.lot?.commodity_product_name_en || option?.lot?.product || null,
                      auto_assign_enabled: Boolean(createForm.auto_assign_lane),
                    });
                  }}
              ...
              <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                ...
                {createForm.auto_assign_lane && lanePreviewError && (
                    ...
                    {sessionOptions.map((s) => (
                      ...
                      >
                        <Stack spacing={0.35} sx={{ py: 0.3, width: "100%" }}>
                          ...
                          {s.rank_meta?.why && (
                            <Typography variant="caption" sx={{ color: s.rank_meta?.isCompatible ? "success.main" : "text.secondary", fontWeight: 600 }}>
                              {s.rank_meta?.isCompatible ? `Best match for selected lot · ${s.rank_meta.why}` : `Lane available · ${s.rank_meta.why}`}
                            </Typography>
                          )}
                        </Stack>
                      </MenuItem>
