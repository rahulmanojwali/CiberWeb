      ...
      <Stack spacing={2}>
        ...
        <Paper sx={{ p: 2 }}>
          <Stack direction={{ xs: "column", md: "row" }} spacing={1.5}>
            ...
            <TextField size="small" label="Language" value={filters.language} onChange={(e) => setFilters((p) => ({ ...p, language: e.target.value }))} />
        ...
        <DialogContent dividers>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
              <TextField label="Event Key" value={form.event_key} onChange={(e) => setForm((p) => ({ ...p, event_key: e.target.value.toUpperCase() }))} fullWidth required />
              <TextField select label="Channel" value={form.channel} onChange={(e) => setForm((p) => ({ ...p, channel: e.target.value }))} sx={{ minWidth: 160 }}>
                <MenuItem value="in_app">In App</MenuItem>
                <MenuItem value="push">Push</MenuItem>
                <MenuItem value="both">Both</MenuItem>
              </TextField>
              <TextField label="Language" value={form.language} onChange={(e) => setForm((p) => ({ ...p, language: e.target.value.toLowerCase() }))} sx={{ minWidth: 120 }} required />
            </Stack>
            <TextField label="Title Template" value={form.title_template} onChange={(e) => setForm((p) => ({ ...p, title_template: e.target.value }))} fullWidth required />
            <TextField label="Body Template" value={form.body_template} onChange={(e) => setForm((p) => ({ ...p, body_template: e.target.value }))} fullWidth multiline minRows={3} required />
            <TextField label="Variables" value={form.variables} onChange={(e) => setForm((p) => ({ ...p, variables: e.target.value }))} helperText="Comma-separated variable names" fullWidth />
            <Stack direction="row" spacing={1} alignItems="center">
              <Switch checked={form.is_active} onChange={(e) => setForm((p) => ({ ...p, is_active: e.target.checked }))} />
              <Typography>Active</Typography>
            </Stack>
            <TextField label="Sample Variables JSON" value={sampleVariables} onChange={(e) => setSampleVariables(e.target.value)} fullWidth multiline minRows={3} />
            <Stack direction={{ xs: "column", md: "row" }} spacing={1}>
              <Button variant="outlined" startIcon={<VisibilityIcon />} onClick={handlePreview}>Preview</Button>
              <TextField label="Test username/mobile" value={testTarget} onChange={(e) => setTestTarget(e.target.value)} size="small" sx={{ minWidth: 220 }} />
              <Button variant="outlined" startIcon={<SendIcon />} onClick={handleTestSend} disabled={!canTest || !testTarget || !form.event_key}>Test Send</Button>
            </Stack>
