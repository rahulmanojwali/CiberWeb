    ...
    >
      ...
      <Card variant="outlined" sx={{ borderColor: BORDER, boxShadow: "none" }}><CardContent>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 4 }}><FormControl fullWidth size="small"><InputLabel>Module</InputLabel><Select label="Module" value={moduleFilter} onChange={(e) => setModuleFilter(String(e.target.value))}>{MODULES.map((m) => <MenuItem key={m} value={m}>{label(m)}</MenuItem>)}</Select></FormControl></Grid>
          <Grid size={{ xs: 12, md: 3 }}><FormControl fullWidth size="small"><InputLabel>Mode</InputLabel><Select label="Mode" value={modeFilter} onChange={(e) => setModeFilter(String(e.target.value))}><MenuItem value="">All</MenuItem>{MODES.map((m) => <MenuItem key={m} value={m}>{m}</MenuItem>)}</Select></FormControl></Grid>
        </Grid>
      </CardContent></Card>

      <Grid container spacing={2}>
        {[['Active Gateways', active], ['Default Gateway', def], ['Test Gateways', testCount], ['Live Gateways', liveCount]].map(([t, v]) => <Grid key={String(t)} size={{ xs: 12, sm: 6, md: 3 }}><Card variant="outlined" sx={{ borderColor: BORDER, boxShadow: "none", bgcolor: "white" }}><CardContent><Typography variant="body2" color="text.secondary">{t}</Typography><Typography variant="h5" sx={{ mt: 1, color: GREEN }}>{String(v)}</Typography></CardContent></Card></Grid>)}
      </Grid>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogContent dividers><Grid container spacing={2} sx={{ pt: 1 }}>
          <Grid size={{ xs: 12, md: 4 }}><FormControl fullWidth><InputLabel>Module</InputLabel><Select label="Module" value={draft.module_code} onChange={(e) => setDraft({ ...draft, module_code: String(e.target.value) })}>{MODULES.map((m) => <MenuItem key={m} value={m}>{label(m)}</MenuItem>)}</Select></FormControl></Grid>
          <Grid size={{ xs: 12, md: 4 }}><FormControl fullWidth><InputLabel>Provider</InputLabel><Select label="Provider" value={draft.provider_code} onChange={(e) => setDraft({ ...draft, provider_code: String(e.target.value) })}>{PROVIDERS.map((p) => <MenuItem key={p} value={p}>{p}</MenuItem>)}</Select></FormControl></Grid>
          <Grid size={{ xs: 12, md: 4 }}><FormControl fullWidth><InputLabel>Mode</InputLabel><Select label="Mode" value={draft.mode} onChange={(e) => setDraft({ ...draft, mode: String(e.target.value) })}>{MODES.map((m) => <MenuItem key={m} value={m}>{m}</MenuItem>)}</Select></FormControl></Grid>
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label={draft.provider_code === 'CCAVENUE' ? 'Merchant ID' : 'Client ID'} value={draft.client_id || ''} onChange={(e) => setDraft({ ...draft, client_id: e.target.value })} /></Grid>
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth type="password" label={draft.provider_code === 'CCAVENUE' ? 'Working Key' : 'Client Secret'} helperText={draft.has_client_secret ? 'Leave blank to keep saved secret.' : ''} value={draft.client_secret || ''} onChange={(e) => setDraft({ ...draft, client_secret: e.target.value })} /></Grid>
          {draft.provider_code === 'CCAVENUE' && <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Access Code" value={draft.access_code || ''} onChange={(e) => setDraft({ ...draft, access_code: e.target.value })} /></Grid>}
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth type="password" label="Webhook Secret" helperText={draft.has_webhook_secret ? 'Leave blank to keep saved secret.' : ''} value={draft.webhook_secret || ''} onChange={(e) => setDraft({ ...draft, webhook_secret: e.target.value })} /></Grid>
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Return URL" value={draft.return_url || ''} onChange={(e) => setDraft({ ...draft, return_url: e.target.value })} /></Grid>
          <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Notify URL" value={draft.notify_url || ''} onChange={(e) => setDraft({ ...draft, notify_url: e.target.value })} /></Grid>
          <Grid size={{ xs: 12, md: 3 }}><TextField fullWidth type="number" label="Priority" value={draft.priority} onChange={(e) => setDraft({ ...draft, priority: Number(e.target.value) })} /></Grid>
          <Grid size={{ xs: 12, md: 3 }}><FormControl fullWidth><InputLabel>Fee Borne By</InputLabel><Select label="Fee Borne By" value={draft.fee_borne_by || 'TRADER'} onChange={(e) => setDraft({ ...draft, fee_borne_by: String(e.target.value) })}><MenuItem value="TRADER">Trader</MenuItem><MenuItem value="PLATFORM">Platform</MenuItem></Select></FormControl></Grid>
