...
function normalizeOrgScopeLabel(line: ChargeLine, scopeType: ScopeType): string {
  if (scopeType === "ORG_ALL_MANDIS" && String(line.charge_owner || "").toUpperCase() === "ORG") {
    if (line.charge_code === "PLATFORM_FEE") return "Organisation Platform Fee";
    if (line.charge_code === "GATEWAY_FEE") return "Organisation Gateway Fee";
  ...
  if (scopeType === "ORG_MANDI_SPECIFIC" && String(line.charge_owner || "").toUpperCase() === "MANDI") {
    if (line.charge_code === "PLATFORM_FEE") return "Mandi Fee";
    if (line.charge_code === "GATEWAY_FEE") return "Mandi Gateway Fee";
...
export const SettlementChargeSettingsPage: React.FC = () => {
  ...
  const [globalScopeLocked, setGlobalScopeLocked] = useState(false);
  ...
  const isOrgEditableScope = !isCiberScope;
  const isLockedByCiberMandiGlobal = !isSuperAdmin && globalScopeLocked && isCiberScope;
  ...
  const isGlobalScopeLockedResponse = (settings: any, effectiveLines: ChargeLine[], readonlyGlobalLines: ChargeLine[]) => {
    if (isSuperAdmin) return false;
    const markers = [
      settings?.scope_type,
      settings?.effective_scope_type,
      settings?.applied_scope_type,
      settings?.charge_scope,
      settings?.inherited_scope_type,
      settings?.inherited_from_scope,
      settings?.resolved_scope_type,
      settings?.source_scope_type,
    ].map((v) => String(v || "").toUpperCase());

    const explicitGlobalMarker = markers.includes("CIBERMANDI_GLOBAL");
    const explicitLockFlag = [
      settings?.global_scope_active,
      settings?.is_global_scope_active,
      settings?.cibermandi_global_active,
      settings?.is_inherited_from_global,
      settings?.locked_by_global,
      settings?.inherited_from_cibermandi_global,
    ].some((v) => v === true || String(v || "").toUpperCase() === "Y" || String(v || "").toUpperCase() === "TRUE");
    const allEffectiveLinesAreCiberMandi =
      effectiveLines.length > 0 && effectiveLines.every((line) => String(line.charge_owner || "").toUpperCase() === "CIBERMANDI");

    return explicitGlobalMarker || explicitLockFlag || allEffectiveLinesAreCiberMandi || readonlyGlobalLines.length > 0;
  };
  ...
  const load = async (force = false) => {
    ...
    try {
      ...
      const displayLines = requestedIsOrgScope
        ? editableLocalLines
        : (lockedByGlobal ? inheritedGlobalLines : scopedNormalizedLines).map((line: ChargeLine) => ({
            ...line,
            charge_owner: "CIBERMANDI" as const,
            editable_by_org: false,
            locked_reason:
              line.locked_reason ||
              "CiberMandi Global scope is active. These values are inherited from the platform and cannot be changed at organisation or mandi level.",
          }));

      setGlobalScopeLocked(lockedByGlobal);
      setForm({
        scope_type: (requestedIsOrgScope ? requestedScope : (settings.scope_type || (isSuperAdmin ? "CIBERMANDI_GLOBAL" : "ORG_ALL_MANDIS"))) as ScopeType,
        ...
        charge_lines: displayLines,
        is_active: settings.is_active !== false,
      });
      setCiberMandiReadonlyLines(inheritedGlobalLines.map((line: ChargeLine) => ({
        ...line,
        charge_owner: "CIBERMANDI" as const,
        editable_by_org: false,
        locked_reason: line.locked_reason || "Managed by CiberMandi and applied globally.",
      })));
      setErrors({});
  ...
  const save = async () => {
    const username = getUsername();
    if (!username) return;
    if (isLockedByCiberMandiGlobal) {
      setApiError("CiberMandi Global charges are platform-owned. Switch to an Organisation scope to add or edit organisation-level additional charges.");
      return;
    }
    const e = validate();
  ...
  return (
    <PageContainer>
      ...
      {!isSuperAdmin && globalScopeLocked && (
        <Alert severity="warning">
          CiberMandi Global charges are inherited and read-only. Organisation admins may still add, edit, enable, or disable their own additional organisation charges in the Organisation Charges section.
        </Alert>
      )}
      ...
      <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between" alignItems={{ xs: "flex-start", md: "center" }}>
        <Typography variant="h6">{!isSuperAdmin && isOrgEditableScope ? "Organisation Charges" : isLockedByCiberMandiGlobal ? "Inherited CiberMandi Global Charges" : "Charge Rules"}</Typography>
